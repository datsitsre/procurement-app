...
function SupplierInvoicesList({ supplier }: { supplier: SupplierProfile }) {
  const router = useRouter();
  const { data: invoices } = useAsyncData<Invoice[]>(supplier.id, () => invoicesService.listInvoicesForSupplier(supplier.id));
            ...
            >
              <div>
                ...
                <p className="text-caption">
                  {allCompanies().find((c) => c.id === inv.companyId)?.name ?? 'Buyer'} · due {formatDate(inv.dueDate)}
                </p>
              </div>
              <div className="flex items-center gap-3">
                <PriceDisplay amount={inv.total} size="sm" />
