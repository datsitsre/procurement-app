...
export const PUT = withErrorHandling("/api/companies/[companyId]/cart/items/[productId]", async (request: NextRequest, ctx: RouteContext<'/api/companies/[companyId]/cart/items/[productId]'>) => {
...
export const DELETE = withErrorHandling("/api/companies/[companyId]/cart/items/[productId]", async (request: NextRequest, ctx: RouteContext<'/api/companies/[companyId]/cart/items/[productId]'>) => {
  const { companyId, productId } = await ctx.params;
  const access = await requireCompanyAccess(request, companyId);
  if (!access.ok) return access.response;
